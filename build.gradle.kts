...
application { mainClass = "org.plumelib.multiversioncontrol.MultiVersionControl" }
...
// Run `./gradlew test -Pregenerate` to overwrite the goal files of the end-to-end tests with the
// program's actual output.  Always inspect the resulting diffs before committing them.
val regenerateGoalFiles = project.hasProperty("regenerate")
...
tasks.withType<Test>().configureEach {
  ...
  // Where to find the end-to-end test cases, and where to write goal files when regenerating them.
  systemProperty("mvc.test.casesDir", file("src/test/resources/e2e").absolutePath)
  if (regenerateGoalFiles) {
    systemProperty("mvc.test.regenerate", "true")
  }

  // Local variables, because a task action that reads `sourceSets`, `jacoco`, or `file()` captures
  // the `Project` object, which the configuration cache forbids.
...
//
// This is an object rather than a function of the build script, because a function of the build
// script is a method of the script object, so a task action that calls it captures the script --
// and thus the "Project" object, which the configuration cache forbids.
object JavadocFonts {
...
val requireJavadoc =
  tasks.register<JavaExec>("requireJavadoc") {
    ...
    // A local variable, because a task action that reads a variable of the build script captures
    // the script -- and thus the "Project" object, which the configuration cache forbids.
    ...
    // Runs only if the tool found no problems, so a failure does not leave behind a marker file
...
tasks.register<Javadoc>("updateUserOptions") {
  ...
  val docletOptions = options as StandardJavadocDocletOptions
  docletOptions.memberLevel = JavadocMemberLevel.PRIVATE
  docletOptions.doclet = "org.plumelib.options.OptionsDoclet"
  docletOptions.addStringOption(
    "docfile",
    "$projectDir/src/main/java/org/plumelib/multiversioncontrol/MultiVersionControl.java",
  )
  docletOptions.addStringOption("format", "javadoc")
  // OptionsDoclet's "-i" means "edit the docfile in place"; it takes no value.
  docletOptions.addBooleanOption("i", true)
  // Overrides the blanket `isNoTimestamp = true` in the `withType<Javadoc>` block above.
  docletOptions.isNoTimestamp = false
  title = ""
  ...
  doFirst { options.docletpath = mainRuntimeClasspath.files.toList() }
}
